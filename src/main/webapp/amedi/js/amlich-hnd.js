...
function jdn(dd, mm, yy) {
    var a = INT((14 - mm*1) / 12);
    var y = yy*1 + 4800 - a*1;
    var m = mm*1 + 12 * a*1 - 3;
    var jd = dd*1 + INT((153 * m + 2) / 5) + 365 * y + INT(y*1 / 4) - INT(y*1 / 100) + INT(y*1 / 400) - 32045;
    return jd;
...
function printToday() {
    getSelectedMonth();
    return printTodayDetail(currentMonth, currentYear);
}
function printTodayDetail(mm, yy) {
    var res = "";
    res += printInputTD(mm, yy);
    return res;
}
function printInputTD(mm, yy) {
    ...
    for (i = 0; i < 6; i++) {
        ...
        for (j = 0; j < 7; j++) {
            ...
            if (k < emptyCells || k >= emptyCells + currentMonth.length) {
                ...
                res += printCellIP(dem, ld1, solar, mm, yy);
            }
            dem++;
        }
...
function printCellIP(dem, lunarDate, solarDate, solarMonth, solarYear) {
    ...
    solarColor = "black";
    var res = "";
    var args = dem + "," + lunarDate.day + "," + lunarDate.month + "," + lunarDate.year + "," + lunarDate.leap + ",30";
    ...
    if (solarDate == today.getDate()) {
        cellClass = "homnay";
        res += '<input id="vitriHomnay" type="hidden" value="' + args + '"/>';
    }
...
function printTable(mm, yy) {
    ...
    var LunarHead = getYearCanChi(ld1.year);
    var res = '';
    res += ('<table class="thang" cellpadding="1" cellspacing="1" width="100%">\n');
    res += printHead(mm, yy);
    var dem = 0;
    for (i = 0; i < 6; i++) {
        res += ("<tr>\n");
        for (j = 0; j < 7; j++) {
            ...
            if (k < emptyCells || k >= emptyCells + currentMonth.length) {
                ...
                solar = k - emptyCells + 1;
                ld1 = currentMonth[k - emptyCells];
                res += printCell(dem, ld1, solar, mm, yy);
            }
            dem++;
        }
        res += ("</tr>\n");
...
function printCell(dem, lunarDate, solarDate, solarMonth, solarYear) {
    var cellClass, solarClass, lunarClass, solarColor;
    cellClass = "ngaythang";
    solarClass = "t2t6";
    lunarClass = "am";
    ...
    res += ('<td class="' + cellClass + '" id="cell' + dem + '"');
    if (lunarDate != null)
        res += (' title="' + getDayName(lunarDate) + '" onClick="showDayInfo(' + args + ');">');
